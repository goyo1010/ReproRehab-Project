 function UM_APDMdataconvert_vG3MAYO
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if sensortype == 2
    ...
    for ii = 1:num_sensors
        ...
        if strcmpi(sensor_label,'Right Arm')
            sensor_label = 'RArm';
        elseif strcmpi(sensor_label,'Left Arm')
            sensor_label = 'LArm';
        elseif strcmpi(sensor_label,'Chest')
            sensor_label = 'torso';
        end
...
%         vectorlengths(ii) = length(IMU.(sensor_label).time);
    ...
    for jj = 1:num_sensors
        ...
        if ~isempty(IMU.(char(sensor_names(jj))).a)
            IMU.(char(sensor_names(jj))).a(delEntries,:) = [];
        end
        if ~isempty(IMU.(char(sensor_names(jj))).w)
            IMU.(char(sensor_names(jj))).w(delEntries,:) = [];
        end
        if ~isempty(IMU.(char(sensor_names(jj))).m)
            IMU.(char(sensor_names(jj))).m(delEntries,:) = [];
        end
        if ~isempty(IMU.(char(sensor_names(jj))).q)
            IMU.(char(sensor_names(jj))).q(delEntries,:) = [];
        end
        if ~isempty(IMU.(char(sensor_names(jj))).b)
            IMU.(char(sensor_names(jj))).b(delEntries) = [];
        end
    ...
    for jj = 1:num_sensors
        ...
        elseif vectorlengths(jj) ~= maxlength
            ...
            if length(IMU.(char(sensor_names(jj))).a) == length(IMU.(char(sensor_names(jj))).time)
                IMU.(char(sensor_names(jj))).a = interp1(double(IMU.(char(sensor_names(jj))).time),IMU.(char(sensor_names(jj))).a,double(IMU.(char(sensor_names(indmax))).time),'linear','extrap');
            else
                IMU.(char(sensor_names(jj))) = rmfield(IMU.(char(sensor_names(jj))),'a');
            end
            if length(IMU.(char(sensor_names(jj))).w) == length(IMU.(char(sensor_names(jj))).time)
                IMU.(char(sensor_names(jj))).w = interp1(double(IMU.(char(sensor_names(jj))).time),IMU.(char(sensor_names(jj))).w,double(IMU.(char(sensor_names(indmax))).time),'linear','extrap');
            else
                IMU.(char(sensor_names(jj))) = rmfield(IMU.(char(sensor_names(jj))),'w');
            end
            if length(IMU.(char(sensor_names(jj))).m) == length(IMU.(char(sensor_names(jj))).time)
                IMU.(char(sensor_names(jj))).m = interp1(double(IMU.(char(sensor_names(jj))).time),IMU.(char(sensor_names(jj))).m,double(IMU.(char(sensor_names(indmax))).time),'linear','extrap');
            else
                IMU.(char(sensor_names(jj))) = rmfield(IMU.(char(sensor_names(jj))),'m');
            end
            if length(IMU.(char(sensor_names(jj))).b) == length(IMU.(char(sensor_names(jj))).time)
                IMU.(char(sensor_names(jj))).b = interp1(double(IMU.(char(sensor_names(jj))).time),IMU.(char(sensor_names(jj))).b,double(IMU.(char(sensor_names(indmax))).time),'linear','extrap');
            else
                IMU.(char(sensor_names(jj))) = rmfield(IMU.(char(sensor_names(jj))),'b');
            end
            if length(IMU.(char(sensor_names(jj))).temp) == length(IMU.(char(sensor_names(jj))).time)      
